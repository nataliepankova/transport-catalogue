...
TransportRouter::TransportRouter(const transport_catalogue::TransportCatalogue& transp_cat, const std::optional<TranspRouteParams>& params)
	:transport_catalogue_(transp_cat),
	graph_(Graph{ transport_catalogue_.GetStops().size() * 2 }),
	router_(nullptr)
{
	if (params) {
		bus_wait_time_ = params->bus_wait_time;
		bus_velocity_ = params->bus_velocity;
	}
	MakeGraph();
...
void TransportRouter::AddStopsToGraph() {
	...
	for (const auto& stop : transport_catalogue_.GetStops()) {
		...
		graph_.AddEdge({ stops_to_vertex_ids_.at(stop.name).stop_wait_id, stops_to_vertex_ids_.at(stop.name).stop_go_id, bus_wait_time_, EdgeType::WAIT, stop.name, 0 });
...
std::optional<TranspRouteInfo> TransportRouter::MakeRoute(const std::string& stop_from, const std::string& stop_to) {
	TranspRouteInfo result;
	if (stop_from == stop_to) {
		return result;
	}
	auto route_info =  router_->BuildRoute(stops_to_vertex_ids_.at(stop_from).stop_wait_id, stops_to_vertex_ids_.at(stop_to).stop_wait_id);
	if (!route_info) {
		return std::nullopt;
	}
	...
	for (const auto& edge : route_info->edges) {
		...
		if (curr_edge_data.type == EdgeType::WAIT) {
			result.items.emplace_back(RouteItemInfo{ EdgeType::WAIT, curr_edge_data.entity_name, 0, bus_wait_time_ });
